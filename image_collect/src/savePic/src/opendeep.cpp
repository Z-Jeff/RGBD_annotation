...
class ImageConverter
{
	...
	public:
		...
		void imageCb(const sensor_msgs::ImageConstPtr& msg){
			...
			cv::Mat scaledMat(cv_ptr->image.rows, cv_ptr->image.cols, CV_16UC1);
			...
			if (m_display){
				...
				cv::waitKey(3);
                        ...
                        cv::imshow(WINDOW, scaledMat);
                        if (waitKey(10) >=0){
				sprintf(path, "./%s%d%s", "deepth", chess_num++, ".png");

                                imwrite(path,scaledMat);
			}
		}
};
